...
class source:
    ...
    def addArticle(self, articleId, articleValidity):
        if not str(articleId) in self.articles:
            self.reputation = (float(self.reputation)*self.size+articleValidity)/(self.size+1)
    ...
    for op in opinions:
        ...
        if op.stance == 0:
            if op.sourceName in globals.sources:
                finalStance -= float(globals.sources.get(op.sourceName).reputation)
        #agree
        if op.stance == 1:
            if op.sourceName in globals.sources:
                finalStance += float(globals.sources.get(op.sourceName).reputation)
        ...
        if op.stance == 3:
            if op.sourceName in globals.sources:
                finalStance += float(globals.sources.get(op.sourceName).reputation)/4.0
